...
const UnAssignedProgramList = ({ programs, teachers }) => {
  const [state, dispatch] = useFormState(assignProgramAction, initialResponse);
      ...
      <form action={dispatch} noValidate>
        ...
        <div className="input-group mb-3 ">
          <div className="form-floating">
            <select
              className={`form-select${isInvalid(state?.errors.teacherId)}`}
              id="teacherId"
              name="teacherId"
            ...
            <div className="invalid-feedback">{state?.errors.teacherId}</div>
          </div>
